...
const RichTextComponents = {
  ...
  list: {
    ...
    number: ({ children }: any) => (
      <ol className='ml-lg py-5 list-decimal space-y-5'>{children}</ol>
    ),
  },
  block: {
    ...
    blockquote: ({ children }: any) => (
      <blockquote className='border-l-[#52daf2] border-l-4 pl-5 py-5 my-5'>{children}</blockquote>
    ),
  },
  marks: {
    link: ({ children, value }: any) => {
      ...
      return (
        <Link
          href={value.href}
          rel={rel}
          className='underline decoration-[#52daf2] hover:decoration-black'>
          {children}
        </Link>
      );
    },
